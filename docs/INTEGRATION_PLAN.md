...
- **GitHub Actions の認証を OIDC（federated credentials）化**（2026-05-03 追加）
  - 現状は App Service の発行プロファイル（基本認証）を Secret に貼って `azure/webapps-deploy@v3` で deploy している
  - Microsoft は本番環境では OIDC を推奨。Entra ID にアプリ登録 + GitHub の OIDC issuer を信頼させる設定で、長寿命 secret を持たずに済む
  - 移行手順：Entra アプリ登録 → 連合資格情報を GitHub repo / branch にバインド → ワークフローに `azure/login@v2` を追加し `webapps-deploy` の `publish-profile` を外す
